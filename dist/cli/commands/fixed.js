...
export function createFixedCommand(getServices) {
  ...
  $ mcp-tool fixed stats interface-name
            ...
            if (options.output === 'json') {
                ...
                interfaces.forEach((iface, index) => {
                    ...
                    if (options.showValidation) {
                        const validationIcon = iface.validationStatus === 'valid' ? '✓' :
                            iface.validationStatus === 'invalid' ? '✗' : '⚠';
                    ...
                    if (showDetailed) {
                        row.push(iface.description || 'N/A', new Date(iface.createdAt).toLocaleDateString(), (iface.lastValidated ? new Date(iface.lastValidated).toLocaleDateString() : 'Never'));
                    }
                    if (options.performance) {
    ...
    fixedCmd
        ...
        try {
            ...
            const result = await interfaceManager.executeInterface(targetInterface.id, parsedParams, executionOptions);
            if (options.output === 'json') {
                console.log(JSON.stringify({
