...
export function setupCollectorCommands(program, getServices) {
    ...
    collectorCmd
        ...
        try {
            ...
            const collector = await services.collectorManager.registerCollector(path, name, options.description, timeout);
            const output = {
                id: collector.id,
                name: collector.name,
                ...
                for (const collector of collectors) {
                    if (collector.enabled) {
                        try {
                            ...
                            if (collectorItem) {
                                collectorItem.test_status = testResult.status;
                                collectorItem.test_error = testResult.error;
                            }
                        }
                        catch (error) {
                            const collectorItem = collectorInfo.find(c => c.id === collector.id);
                            if (collectorItem) {
                                collectorItem.test_status = 'error';
                                collectorItem.test_error = error instanceof Error ? error.message : String(error);
                            }
                        }
                    }
                }
    ...
    collectorCmd
        ...
        try {
            ...
            const result = await services.collectorManager.executeCollector(name, input, executionOptions);
