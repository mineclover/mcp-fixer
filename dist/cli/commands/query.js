...
export function setupQueryCommands(program, getServices) {
    ...
    queryCmd
        ...
        try {
            ...
            if (!toolId) {
                ...
                const tools = services.db.prepare('SELECT id, name FROM tools WHERE status = ? ORDER BY name').all('active');
                if (tools.length === 0) {
    ...
    queryCmd
        ...
        try {
            ...
            if (options.tool) {
                // Resolve tool ID
                const tool = services.db.prepare('SELECT id FROM tools WHERE id = ? OR name = ?').get(options.tool, options.tool);
                if (!tool) {
            ...
            const enhancedQueries = await Promise.all(result.queries.map(async (query) => {
                const tool = services.db.prepare('SELECT name FROM tools WHERE id = ?').get(query.toolId);
                return {
                    id: query.id,
                    name: query.name,
    ...
    queryCmd
        ...
        .action(async (queryName, options, command) => {
        const globalOpts = command.parent?.parent?.opts();
        const formatter = new OutputFormatter(globalOpts.format, globalOpts.quiet);
        try {
            const services = await getServices(globalOpts);
            ...
            if (options.collector) {
                ...
                const collectorResult = await services.collectorManager.executeCollector(options.collector, parameters);
            ...
            const result = await services.queryEngine.executeQuery(query.id, parameters, collectorInputs, executionOptions);
