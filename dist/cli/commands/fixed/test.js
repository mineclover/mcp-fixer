...
export async function executeTestCommand(interfaceName, interfaceManager, options) {
    try {
...
async function testSingleInterface(interfaceName, interfaceManager, options) {
    ...
    if (options.benchmark) {
        ...
        const benchmarkResults = await runPerformanceBenchmark(targetInterface, interfaceManager, options);
...
async function testAllInterfaces(interfaceManager, options) {
...
async function runComprehensiveTestSuite(interfaceName, interfaceManager, options) {
...
async function runPerformanceBenchmark(targetInterface, interfaceManager, options) {
    const iterations = 10; // Simplified for demo
    let totalTime = 0;
    let successCount = 0;
    for (let i = 0; i < iterations; i++) {
        try {
            const result = await interfaceManager.executeInterface(targetInterface.id, {}, { timeout: 30000, recordMetrics: false });
...
async function generateTestReport(testResults, filename) {
    const reportContent = `
<!DOCTYPE html>
<html>
<head>
