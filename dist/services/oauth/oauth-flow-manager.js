 ...
 */
import { OAuthConfigurationModel, OAuthTokenModel } from '../../models/index.js';
export class OAuthFlowManager {
    ...
    async createConfiguration(configData) {
        ...
        const existing = await this.findConfigurationByTool(configData.toolId, configData.providerName);
        if (existing) {
    ...
    `);
        stmt.run(oauthConfig.id, oauthConfig.toolId, oauthConfig.providerName, oauthConfig.authorizationUrl, oauthConfig.tokenUrl, oauthConfig.clientId, JSON.stringify(oauthConfig.scopes), JSON.stringify(oauthConfig.additionalParams), oauthConfig.createdAt, oauthConfig.updatedAt);
        return oauthConfig;
    }
    /**
     * Initiate OAuth authorization flow with manual intervention detection
     */
    async initiateAuthFlow(toolId, redirectUri, scopes) {
     ...
     */
    async handleCallback(authorizationCode, state, redirectUri) {
        ...
        try {
            ...
            const token = await OAuthTokenModel.createFromAuthResponse(config.id, tokenResponse, this.config.encryptionKey);
     ...
     */
    async refreshToken(toolId, options = {}) {
        ...
        try {
            ...
            const refreshToken = await OAuthTokenModel.decryptToken(token.refreshTokenEncrypted, this.config.encryptionKey);
            // Make refresh request
            const refreshResponse = await this.makeTokenRefreshRequest(config, refreshToken);
            // Create new token
            const newToken = await OAuthTokenModel.createFromAuthResponse(config.id, refreshResponse, this.config.encryptionKey);
            // Update database
    ...
    async getAuthStatus(toolId) {
        ...
        return {
            ...
            token: OAuthTokenModel.sanitize(token),
     ...
     */
    async testAuthentication(toolId) {
        try {
    ...
    async refreshExpiredTokens() {
        const thresholdTime = new Date(Date.now() + this.config.tokenRefreshThreshold * 1000).toISOString();
        const stmt = this.db.prepare(`
    ...
    `);
        const expiredTokens = stmt.all(thresholdTime);
    ...
    detectManualIntervention(config, authorizationUrl) {
    ...
    async attemptAutomatedAuth(authorizationUrl, authRequest) {
    ...
    async exchangeCodeForTokens(config, request) {
    ...
    async makeTokenRefreshRequest(config, refreshToken) {
    ...
    async findConfigurationByTool(toolId, providerName) {
        const stmt = this.db.prepare(`
      SELECT * FROM oauth_configurations 
      WHERE tool_id = ? AND provider_name = ?
    `);
        const row = stmt.get(toolId, providerName);
        return row ? OAuthConfigurationModel.fromDatabase(row) : null;
    }
    async getTokenByTool(toolId) {
        const stmt = this.db.prepare(`
    ...
    `);
        stmt.run(token.id, token.oauthConfigId, token.accessTokenEncrypted, token.refreshTokenEncrypted || null, token.tokenType, token.expiresAt || null, token.scope || null, token.createdAt);
    }
