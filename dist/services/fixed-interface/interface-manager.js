 ...
 */
import { FixedInterfaceModel, PerformanceMetricModel, ToolModel } from '../../models/index.js';
import Ajv from 'ajv';
export class FixedInterfaceManager {
     ...
     */
    async registerInterface(interfaceData, options = {}) {
        ...
        if (options.autoDiscover) {
            const discoveredSchema = await this.discoverInterfaceSchema(interfaceData.toolId, interfaceData.name);
        ...
        const existingInterface = await this.findInterfaceByName(interfaceData.toolId, interfaceData.name);
      ...
      `);
            stmt.run(fixedInterface.id, fixedInterface.toolId, fixedInterface.name, fixedInterface.displayName, fixedInterface.description || null, JSON.stringify(fixedInterface.schemaJson), JSON.stringify(fixedInterface.parametersJson), JSON.stringify(fixedInterface.responseSchemaJson), fixedInterface.version, fixedInterface.isActive ? 1 : 0, fixedInterface.createdAt);
        }
        // Update cache
     ...
     */
    async listInterfaces(toolId, filters = {}) {
        let query = `
    ...
    `);
        const row = stmt.get(interfaceId);
        if (!row) {
            return null;
        }
     ...
     */
    async executeInterface(interfaceId, parameters, options = {}) {
        ...
        try {
            ...
            const executionResult = await this.executeMCPOperation(tool, fixedInterface, parameters, options);
            ...
            if (options.recordMetrics !== false && this.config.enableMetrics) {
                await this.recordPerformanceMetric(fixedInterface, responseTime, executionResult.success, executionResult.error);
            }
            return {
            ...
            if (options.recordMetrics !== false && this.config.enableMetrics) {
                const fixedInterface = await this.getInterface(interfaceId);
                if (fixedInterface) {
                    await this.recordPerformanceMetric(fixedInterface, responseTime, false, errorMessage);
                }
            }
            return {
                success: false,
     ...
     */
    async validateInterface(interfaceId) {
        // Check cache first
     ...
     * Update interface
     */
    async updateInterface(interfaceId, updateData) {
      ...
      WHERE id = ?
    `);
        stmt.run(updatedInterface.displayName, updatedInterface.description || null, JSON.stringify(updatedInterface.schemaJson), JSON.stringify(updatedInterface.parametersJson), JSON.stringify(updatedInterface.responseSchemaJson), updatedInterface.version, updatedInterface.isActive ? 1 : 0, interfaceId);
        // Update cache
     ...
     */
    async getInterfaceStats(toolId, timeRangeHours = 24) {
        ...
        const activeResult = this.db.prepare(activeQuery).get(...params);
    ...
    `;
        ...
        const perfResult = this.db.prepare(perfQuery).get(...perfParams);
        return {
    ...
    async validateToolExists(toolId) {
    ...
    async discoverInterfaceSchema(toolId, operationName) {
    ...
    async findInterfaceByName(toolId, name) {
        const stmt = this.db.prepare(`
      SELECT * FROM fixed_interfaces 
      WHERE tool_id = ? AND name = ?
    `);
        const row = stmt.get(toolId, name);
        return row ? FixedInterfaceModel.fromDatabase(row) : null;
    }
    async validateSchemas(interfaceData) {
        try {
    ...
    validateParameters(fixedInterface, parameters) {
        try {
    ...
    validateResponse(fixedInterface, responseData) {
        try {
    ...
    async getToolForInterface(toolId) {
        const stmt = this.db.prepare(`SELECT * FROM tools WHERE id = ?`);
        const row = stmt.get(toolId);
        return row ? ToolModel.fromDatabase(row) : null;
    }
    async executeMCPOperation(tool, fixedInterface, parameters, options) {
    ...
    async recordPerformanceMetric(fixedInterface, responseTime, success, error) {
        const metric = PerformanceMetricModel.recordOperation(fixedInterface.toolId, fixedInterface.name, 'fixed', responseTime, success, fixedInterface.id, error ? { message: error, category: 'server' } : undefined);
        const stmt = this.db.prepare(`
      INSERT INTO performance_metrics (
        id, interface_id, tool_id, access_type, operation_name,
        response_time_ms, success, error_message, error_category,
        timestamp, metadata
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(metric.id, metric.interfaceId || null, metric.toolId, metric.accessType, metric.operationName, metric.responseTimeMs, metric.success ? 1 : 0, metric.errorMessage || null, metric.errorCategory || null, metric.timestamp, JSON.stringify(metric.metadata));
    }
    async validateAgainstTool(fixedInterface) {
        // This would validate the interface against the actual MCP tool
        // Check if operation exists, parameters are correct, etc.
        // For now, assume validation passes
    }
    async updateLastValidated(interfaceId) {
        const stmt = this.db.prepare(`
    ...
    invalidateValidationCache(interfaceId) {
