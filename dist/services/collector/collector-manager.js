...
import { CollectorModel } from '../../models/collector.js';
...
export class CollectorManager {
     ...
     */
    async registerCollector(filePath, name, description, timeout = 30) {
        try {
      ...
      `);
            stmt.run(dbData.id, dbData.name, dbData.description, dbData.file_path, dbData.input_schema, dbData.output_schema, dbData.timeout, dbData.enabled ? 1 : 0, dbData.version, dbData.created_at, dbData.last_executed, dbData.execution_count);
            return collector;
        }
        catch (error) {
     ...
     */
    async listCollectors(enabledOnly = false) {
        try {
     ...
     */
    async executeCollector(identifier, input = {}, options = {}) {
     ...
     */
    async testCollector(identifier) {
        try {
            const collector = await this.getCollector(identifier);
            if (!collector) {
                throw new Error(`Collector '${identifier}' not found`);
     ...
     */
    async setCollectorEnabled(identifier, enabled) {
        try {
            const collector = await this.getCollector(identifier);
            if (!collector) {
                return false;
     ...
     * Remove a collector
     */
    async removeCollector(identifier) {
        try {
            const collector = await this.getCollector(identifier);
            if (!collector) {
                return false;
    ...
    async getCollectorStats() {
        try {
            const totalResult = this.db.prepare('SELECT COUNT(*) as count FROM collectors').get();
            const enabledResult = this.db.prepare('SELECT COUNT(*) as count FROM collectors WHERE enabled = 1').get();
    ...
    async validateCollectorFile(filePath) {
        const errors = [];
        const warnings = [];
        try {
    ...
    async extractCollectorSchemas(filePath) {
        const ext = extname(filePath).toLowerCase();
        // Default schemas
    ...
    validateCollectorInput(collector, input) {
        try {
    ...
    async executeCollectorFile(collector, input, options, executionId) {
    ...
    async updateCollectorStatistics(collectorId) {
        try {
            const stmt = this.db.prepare(`
