...
export class ToolDiscoveryService {
     ...
     */
    async discoverFromEndpoint(endpoint, options = {}) {
     ...
     */
    async discoverFromEndpoints(endpoints, options = {}) {
        const opts = DiscoveryOptionsSchema.parse({
            ...this.config.discovery,
            ...options,
        });
        ...
        for (const batch of batches) {
            const batchPromises = batch.map(endpoint => this.discoverFromEndpoint(endpoint, opts).catch(error => ({
                tools: [],
                endpoint,
                status: 'error',
                error: error.message,
    ...
    async saveTools(tools) {
        ...
        for (const toolData of tools) {
            try {
                ...
                const existing = this.db.prepare('SELECT id FROM tools WHERE name = ? AND endpoint = ?').get(toolData.name, toolData.endpoint);
                if (existing) {
                    // Update existing tool
                    const updatedTool = await this.updateExistingTool(existing.id, toolData);
                    if (updatedTool) {
          ...
          `);
                    stmt.run(dbData.id, dbData.name, dbData.version, dbData.description, dbData.endpoint, dbData.capabilities, dbData.auth_config, dbData.schema, dbData.discovery_data, dbData.status, dbData.last_checked, dbData.created_at, dbData.updated_at);
                    saved.push(newTool);
                }
            }
            catch (error) {
     ...
     */
    async refreshToolCapabilities(toolIds) {
        const refreshed = [];
        const errors = [];
        try {
            ...
            const tools = this.db.prepare(query).all(...params);
            for (const toolRow of tools) {
                ...
                WHERE id = ?
              `);
                            stmt.run(dbData.capabilities, dbData.auth_config, dbData.schema, dbData.last_checked, dbData.updated_at, dbData.id);
    ...
    async performDiscovery(endpoint, options) {
        const startTime = Date.now();
        try {
    ...
    async discoverWithRetry(endpoint, options) {
        let lastError = null;
    ...
    async makeDiscoveryRequest(endpoint, options) {
    ...
    parseMCPCapabilities(endpoint, data) {
        const tools = [];
        try {
    ...
    async updateExistingTool(toolId, toolData) {
        try {
            const existingRow = this.db.prepare('SELECT * FROM tools WHERE id = ?').get(toolId);
            if (!existingRow) {
                return null;
            }
            const existingTool = ToolModel.fromDatabase(existingRow);
            // Check if update is needed
            const needsUpdate = existingTool.version !== toolData.version ||
        ...
        WHERE id = ?
      `);
            stmt.run(dbData.version, dbData.description, dbData.capabilities, dbData.auth_config, dbData.schema, dbData.discovery_data, dbData.last_checked, dbData.updated_at, dbData.id);
            return updatedTool;
        }
        catch (error) {
