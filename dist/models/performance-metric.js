...
export class PerformanceMetricModel {
     ...
     */
    static validate(metric) {
        try {
            PerformanceMetricSchema.parse(metric);
            const perfMetric = metric;
            const warnings = [];
     ...
     */
    static recordOperation(toolId, operationName, accessType, responseTimeMs, success, interfaceId, error, metadata) {
        return this.create({
            toolId,
            operationName,
            accessType,
     ...
     */
    static calculateAnalytics(metrics, startTime, endTime) {
        if (metrics.length === 0) {
            return {
        ...
        metrics
            ...
            .forEach(m => {
            errorBreakdown[m.errorCategory]++;
        });
     ...
     */
    static compareAccessTypes(toolId, toolName, operationName, metrics) {
     ...
     */
    static generateAlerts(metrics, thresholds = this.DEFAULT_THRESHOLDS) {
        const alerts = [];
     ...
     */
    static validatePerformanceTarget(metrics, thresholds = this.DEFAULT_THRESHOLDS) {
        if (metrics.length === 0) {
            return {
                meetsTarget: false,
     ...
     */
    static generateTrendData(metrics, startTime, endTime) {
     ...
     */
    static calculateStatisticalSignificance(fixedMetrics, dynamicMetrics) {
