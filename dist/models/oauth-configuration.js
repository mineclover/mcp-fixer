...
export class OAuthConfigurationModel {
     ...
     */
    static validateCreate(configData) {
        try {
     ...
     */
    static validateUrls(authUrl, tokenUrl) {
        try {
            const authUrlObj = new URL(authUrl);
            const tokenUrlObj = new URL(tokenUrl);
            return (authUrlObj.protocol === 'https:' &&
    ...
    static validateScopes(scopes) {
        ...
        return (scopes.length <= 20 && // Reasonable scope limit
     ...
     */
    static update(existingConfig, updateData) {
        return {
            ...existingConfig,
            ...updateData,
     ...
     */
    static createFromProvider(toolId, provider, clientId, customScopes) {
     ...
     */
    static buildAuthorizationUrl(config, request) {
