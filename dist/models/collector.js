...
export class CollectorModel {
     ...
     */
    static validateCreate(collectorData) {
        try {
     ...
     */
    static async validateModule(filePath) {
        const errors = [];
        const warnings = [];
        ...
        try {
            ...
            if (!validationResult.success) {
                return {
                    valid: false,
                    errors: validationResult.error.errors.map(e => `Module ${e.path.join('.')}: ${e.message}`),
                    warnings,
                };
            }
     ...
     */
    static validateInput(collector, input) {
        try {
     ...
     */
    static validateOutput(collector, output) {
        try {
     ...
     */
    static update(existingCollector, updateData) {
        return {
            ...existingCollector,
            ...updateData,
        };
     ...
     */
    static fromModule(filePath, module) {
        return {
            name: module.name,
     ...
     */
    static checkDependencies(collector, availableCollectors) {
        const errors = [];
        const warnings = [];
     ...
     */
    static resolveExecutionOrder(collectors) {
        const result = [];
     ...
     */
    static createContext(collector, input, executionId) {
        return {
