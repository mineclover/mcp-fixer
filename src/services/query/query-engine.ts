import { z } from 'zod';
import { Query, QueryModel, QueryCreate, QueryUpdate } from '../../models/query.js';
import { Execution, ExecutionModel, ExecutionCreate } from '../../models/execution.js';
...
export class QueryEngine {
   ...
   * Create a new query
   */
  async createQuery(queryData: QueryCreate): Promise<Query> {
    try {
      // Validate query data
      ...
      stmt.run(
        dbData.id, dbData.name, dbData.description, dbData.tool_id, dbData.schema_version,
        dbData.parameters, dbData.operation, dbData.output_schema, dbData.created_at,
        dbData.updated_at, dbData.execution_count, dbData.last_executed
      );
   ...
   */
  async getQuery(identifier: string): Promise<Query | null> {
    try {
   ...
   */
  async listQueries(filters: QuerySearchFilters = {}, limit = 100, offset = 0): Promise<{
    queries: Query[];
    total: number;
    hasMore: boolean;
  }> {
   ...
   */
  async updateQuery(queryId: string, updates: QueryUpdate): Promise<Query> {
    try {
      // Get existing query
      ...
      stmt.run(
        dbData.name, dbData.description, dbData.parameters, dbData.operation,
        dbData.output_schema, dbData.updated_at, dbData.id
      );
  ...
  async executeQuery(
    ...
    collectorInputs: Record<string, any> = {},
  ...
  async getExecutionHistory(
    ...
    try {
      ...
      const countResult = this.db.prepare(
        'SELECT COUNT(*) as count FROM executions WHERE query_id = ?'
      ).get(queryId) as { count: number };
      ...
      const rows = this.db.prepare(`
        ...
        LIMIT ? OFFSET ?
      `).all(queryId, limit, offset) as any[];
  ...
  private validateQueryParameters(query: Query, parameters: Record<string, any>): {
    valid: boolean;
    errors: string[];
  } {
    try {
  ...
  private async createExecutionRecord(
    ...
    stmt.run(
      dbData.id, dbData.query_id, dbData.parameters, dbData.collector_inputs,
      dbData.credential_refs, dbData.status, dbData.started_at,
      dbData.completed_at, dbData.result, dbData.error_details
    );
  }
  ...
  private async updateQueryStatistics(queryId: string, lastExecuted: string): Promise<void> {
    const stmt = this.db.prepare(`
      UPDATE queries SET 
