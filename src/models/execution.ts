...
export class ExecutionModel {
   ...
   */
  static validate(execution: unknown): { valid: boolean; errors: string[] } {
    try {
  ...
  static fail(execution: Execution, error: Error | string): Execution {
    const errorDetails: ExecutionError = {
  ...
  static validateParameters(
    execution: Execution,
    queryParameterSchema: Record<string, any>
  ...
  static groupByStatus(executions: Execution[]): Record<ExecutionStatusType, number> {
    const groups: Record<ExecutionStatusType, number> = {
      pending: 0,
      running: 0,
      completed: 0,
      failed: 0,
  ...
  static calculateStats(executions: Execution[]): ExecutionStats {
    ...
    let executionsPerHour: number | undefined;
