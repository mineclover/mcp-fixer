...
export const fixedListCommand = new Command('list')
  ...
  .action(async (toolId: string | undefined, options: any) => {
    try {
      ...
      if (options.validate) {
        for (const iface of interfaces) {
          ...
          (iface as any).validationStatus = validation.isValid ? 'valid' : 'invalid';
          (iface as any).validationErrors = validation.errors;
        }
      }
      ...
      if (options.output === 'json') {
        ...
        const outputData = interfaces.map(iface => {
          ...
          if (options.validate) {
            Object.assign(base, {
              status: (iface as any).validationStatus,
              errors: (iface as any).validationErrors?.join('; ') || '-'
            });
          }
