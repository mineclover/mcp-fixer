...
import { AuthType, ApiKeyData, BearerTokenData, BasicAuthData, OAuthData } from '../../services/auth/auth-manager.js';

export function setupAuthCommands(
  program: Command,
  ...
  const authCmd = program
    ...
    .action(async (toolId: string, options, command) => {
      ...
      try {
        ...
        const credential = await services.authManager.storeCredentials(
          actualToolId,
          authType,
          credentialData,
          expiresAt
  ...
  async function promptForAuthType(): Promise<AuthType> {
    ...
    return answers.authType as AuthType;
  }

  async function promptForCredentials(authType: AuthType): Promise<any> {
    switch (authType) {
      case 'api_key': {
        const answers = await inquirer.prompt([
          {
